...
export const HELP = `grimoire ${VERSION} — documentation retrieval for AI agents
  ...
  grimoire staff source <product|id> [--include <pattern>]... [--exclude <pattern>]... [--status active|disabled] [--markdown on|off] [--selector <css>|none] [--executor worker|queue] [--rate-delay <seconds>] [--sitemap <url>]
  ...
  grimoire staff recrawl <product|id> [--failed] | index <product|id> [--priority 0-10] | purge <product|id> --yes
...
export const COMMAND_FLAGS: Record<string, readonly string[]> = {
  ...
  jobs: ["watch"],
  staff: ["reason", "name", "quota", "json", "source", "state", "kind", "limit", "include", "exclude", "status", "markdown", "selector", "executor", "rate-delay", "sitemap", "yes", "watch", "failed", "priority", "rolling", "fixed", "npm", "pypi", "github", "tag-pattern", "staff", "product"],
};
