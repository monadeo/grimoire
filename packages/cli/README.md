...
| `grimoire staff workers [--json]` | Worker processes seen in the last day: version, last heartbeat, the job each holds and when its last batch ended. SILENT marks one that stopped reporting |
...
| `grimoire staff source <product\|id> [--include <pattern>]... [--exclude <pattern>]... [--status active\|disabled] [--markdown on\|off] [--selector <css>\|none]` | Change what a source covers and how it is fetched. `--selector` names the element that is the page's content, for example `.markdown-body` on GitHub pages; `none` clears it. Takes effect at the next reindex |
